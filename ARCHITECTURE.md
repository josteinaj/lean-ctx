...
flowchart TB
    ...
    subgraph dispatch [Dispatch and Governance]
        ...
        HybridDispatch["Hybrid Dispatch — Context Server (68 tools)"]
        ToolRegistry["ToolRegistry — 68 trait-based tools (McpTool)"]
    ...
    HybridDispatch -->|"registry (68 tools)"| ToolRegistry
...
flowchart TD
    ...
    DispatchCall["dispatch_inner — ToolRegistry lookup (68 tools)"]
...
| `server/registry.rs` | `ToolRegistry` — HashMap-based tool lookup, `build_registry()` registers all 68 trait-based tools |
| `server/dispatch/mod.rs` | Registry dispatch — `dispatch_inner` resolves every tool through the `ToolRegistry` (68 tools); `ctx_call` routes meta-invocations back through it |
...
| `tools/registered/` | 68 self-contained `McpTool` implementations (schema + handler co-located) |
